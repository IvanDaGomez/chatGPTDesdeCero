...
def create_attention_mask(x: torch.Tensor) -> torch.Tensor:
  batch, sequence_length, n_dim,  = x.size()[:3]
  ones_tensor = torch.ones((batch, sequence_length, n_dim, n_dim))
  mask = torch.tril(ones_tensor)
  ceros_mask = mask == 0
  mask[ceros_mask] = - torch.inf
  ones_mask = mask == 1 
  mask[ones_mask] = 0
  masked_tensor = x + mask
  final_tensor = F.softmax(masked_tensor, dim=-1)
  return final_tensor
...
def scaled_dot_product(q, k, v, mask=False):
    ...
    if mask:
      mask = create_attention_mask(scaled)
